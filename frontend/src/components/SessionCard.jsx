const SessionCard = ({ session, onClick, onDelete }) => {
    ...
    return (
        <div onClick={() => onClick(session)} className='group bg-white border border-slate-100 p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 transition-all hover:shadow-lg active:scale-[0.98] cursor-pointer'>
            <div className='flex items-center flex-grow w-full gap-4 sm:gap-6 md:w-auto'>
                ...
                <div className='overflow-hidden'>
                    <h3 className='text-base font-bold truncate text-slate-900 sm:text-lg group-hover:text-teal-600'>{session.role}</h3>
            ...
            <div className='flex items-center justify-between w-full gap-6 pt-3 border-t md:justify-end md:w-auto md:border-t-0 md:pt-0'>
                ...
                <div className='flex flex-col items-end gap-1.5'>
                    ...
                    <span className='flex items-center text-xs font-bold text-teal-600'>{session.status === 'completed' ? 'Results' : 'Resume'}
            ...
            <div className='hidden w-px h-10 mx-2 md:block bg-slate-100'></div>

            <button onClick={(e) => { e.stopPropagation(); if (isDeletable) onDelete(e, session._id) }} className='p-3 transition-all text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl' title='Delete Session'>
